							...
							storageConfigFieldName)
{
	...
	function StyleInputFileAndAddFlash(inputFile)
	{
		...
		else
		{
			...
			//alert(replacementDiv.innerHTML);
		...
		function MoveAndResizeDivAndAddFlash()
		{
			...
			var replaceIvan = document.createElement("A");
			replaceIvan.innerHtml = "选择文件";
			replaceIvan.href = "#";

			replacementDiv.lastChild.style.display = "none";
			replacementDiv.insertAdjacentHTML("beforeEnd","<a href='#' style='position:absolute;top:4px;color:#0099ff'>请选择文件</a>");
	       
	        //alert(document.getElementById("NeatUploadDiv_UCFile_multiFile").innerHTML);
		    //alert(replacementDiv.innerHTML);
...
NeatUploadMultiFile.prototype.OnFileQueued = function(file) {
    ...
    {
        file.Delete();
        alert("单个附件大小不可超过200M!");
        return false;
    }
    
    var fTable = document.getElementById("Files_Table");

    var tr = fTable.insertRow(-1);

    tr.style.fontSize = '12px';
    var RowNum = fTable.rows.length - 1;
    ...
    if (file.type == undefined || file.type == null) {
        img.setAttribute('src', '../../NeatUpload/images/icgen.gif');
    }
    else {

        var fileType = (file.type).substring(1, (file.type).length);
        if (fileType == "doc" || fileType == "docx" || fileType == "xls" || fileType == "xls" || fileType == "ppt" || fileType == "pdf" || fileType == "txt" || fileType == "jpg" || fileType == "gif")
        { }
        else
        { fileType = "gen" }
        img.setAttribute('src', '../../NeatUpload/images/ic' + fileType + '.gif');
    ...
    if (file.size > 0 && file.size < 1024)
        fileSize = (file.size).toFixed("2") + "字节";
        ...
        fileSize = "-";
    //span.appendChild(document.createTextNode(' ' + file.name+'('+fileSize+')'));
    td3.appendChild(document.createTextNode(' ' + fileSize));
    tr.appendChild(td3);
    var td4 = document.createElement('td');
    td4.style.color = "#757575";
    ...
    link5.onclick = function() {
        ...
        //RowNum=RowNum-1;
        var tbRows = document.getElementsByName("tb_rows");
        for (var i = 0; i < tbRows.length; i++) {
            tbRows[i].innerText = i + 1;
        }
        //alert(tbRows.length);
        return false;
    };
    link5.appendChild(document.createTextNode('清除'));
    td5.appendChild(link5);
    tr.appendChild(td5);

    var td6 = document.createElement('td');
    td6.style.color = "#757575";
    td6.appendChild(document.createTextNode('未上传'));
    tr.appendChild(td6);
